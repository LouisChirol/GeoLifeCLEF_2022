...
def training():
    ...
    df_train_faune = df_train_faune[df_train_faune.species_id.isin(unique[np.argsort(_counts)[::-1][:500]]).values]
    
    unique, _counts = np.unique(df_train_flore.species_id, return_counts=True)
    df_train_flore = df_train_flore[df_train_flore.species_id.isin(unique[np.argsort(_counts)[::-1][:500]]).values]



    def kingdom_training(df_train, df_val, export_folder="./models/"):
        ...
        model.stem = nn.Conv2d(4, 48, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=True)
        ...
        optimizer = optim.Adam(model.parameters(), lr=0.001)  #they take 0.01
        ...
        for epoch in range(epochs):    
            ...
            export_model = export_folder + "model_resnet_rgnir_epoch_"+str(epoch)+".pth"
            ...
            export_loss_png = export_folder+"loss_resnet_rgnir_epochs_"+str(epochs)+"_batch_size_"+str(batch_size)+".png"
    ...
    kingdom_training(df_train_faune, df_val_faune, export_folder="./models/faune/")

    print("|||| FLORA TRAINING ||||")
    kingdom_training(df_train_flore, df_val_flore, export_folder="./models/flore/")

...
class ImageData(Dataset):
    def __init__(self, df, df_env, data_path, load, landcover_mapping):#, transform):
        ...
        self.transform1 = torchvision.transforms.RandomVerticalFlip(p=0.5)
        self.transform2 = torchvision.transforms.RandomRotation(90)
    ...
    def __getitem__(self, idx):
        ...
        return patch, torch.FloatTensor(env_vect), torch.FloatTensor([species])
...
def training():
    ...
    if class_bal:
        with open("./df_train_augmented_20.pkl", "rb") as f:
            df_train = pkl.load(f)
            df_train['kingdom'] = df_train.apply(lambda x: species[x.species_id], axis=1)


    # Separating faune/flore
    ...
    # _, counts = np.unique(df_train_faune.species_id, return_counts=True)
    # # class_weights_faune = np.sum(counts)/counts
    # class_weights_faune = np.where(counts>aug_th, counts, aug_th)
    # num_samples_faune = int(np.sum(class_weights_faune))

    # _, counts = np.unique(df_train_flore.species_id, return_counts=True)
    # # class_weights_flore = np.sum(counts)/counts
    # class_weights_flore = np.where(counts>aug_th, counts, aug_th)
    # num_samples_flore = int(np.sum(class_weights_flore)) # maybe to increase
    
    # # Sampler
    # sampler_faune = utils.WeightedRandomSampler(class_weights_faune, num_samples_faune)
    # sampler_flore = utils.WeightedRandomSampler(class_weights_flore, num_samples_flore)
    ...
    def kingdom_training(df_train, df_val, df_env, export_folder="./models/", es=10000, es_th=early_stop_th, lr=0.01):
        ...
        # normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406, 0.450, 0.450],
        ...
        # Put the model in training mode (dropout layers unfreezed)
            ...
            export_model = export_folder + "model_resnet18_rgb_epoch_"+str(epoch)+ ".pth"
            ...
            export_loss_png = export_folder+"loss_resnet18_rgb_epochs_"+str(epochs)+"_batch_size_"+str(batch_size)+".png"
    ...
    kingdom_training(df_train_faune, df_val_faune, df_env, export_folder="./models/faune/", es=1000000, lr=0.01)

    print("|||| FLORA TRAINING ||||")
    kingdom_training(df_train_flore, df_val_flore, df_env, export_folder="./models/flore/",es=100000, lr=0.01)
...
def prediction():
    ...
    test_data = ImageDataTest(df = df_test, df_env=df_env, data_path = DATA_PATH, load = load_patch, landcover_mapping=landcover_mapping)
    ...
    with tqdm(enumerate(test_loader), total=total_test) as tepoch_test:
        ...
        # Turn the predictions into an integer array
        preds = np.array(preds).astype(np.int32)



    # Generate the submission file
    generate_submission_file(SUBMISSION_PATH + "resnet_submission.csv", df_obs_test.index[:preds.shape[0]], preds)

...
def training():
    ...
    class MyModel(nn.Module):
        def __init__(self):
            ...
            self.cnn.stem = nn.Conv2d(5, 48, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=True)
    ...
    def kingdom_training(df_train, df_val, df_env, export_folder="./models/", es=10000, es_th=early_stop_th, lr=0.01):
        ...
        # Put the model in training mode (dropout layers unfreezed)
            ...
            export_model = export_folder + "model_effnet_rgnirland_sch_epoch_"+str(epoch)+str(nb_classes)+"_classes"+ ".pth"
            ...
            export_loss_png = export_folder+"loss_effnet_rgnirland_sch_epochs_"+str(epochs)+"_batch_size_"+str(batch_size)+ str(nb_classes) + "_classes"+".png"
...
def prediction():
    ...
    # Load models
    model_faune.cnn.load_state_dict(torch.load("./models/faune/model_effnet_rgnirland_sch_epoch_31500_classes.pth").state_dict())
    model_flore.cnn.load_state_dict(torch.load("./models/flore/model_effnet_rgnirland_sch_epoch_31500_classes.pth").state_dict())
    ...
    generate_submission_file(SUBMISSION_PATH + "efficient_net_4_rgnirland_500_classes_epochs.csv", df_obs_test.index[:preds.shape[0]], preds)
